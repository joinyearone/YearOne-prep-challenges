...
const postQuestion = async (question) => {
  ...
  // url creation
  const post_url = `${CIRCLE_COMMUNITY_PATH}/api/v1/posts?community_id=${
      COMMUNITY_ID}&SPACE_ID=${SPACE_ID}&`;
  const post_title = `name=${title}&`;
  const post_body = `body=${body}&`;
  const post_url_ending_params =
      `is_comments_enabled=true&is_liking_enabled=true&is_truncation_disabled=true`;
  const post_author = `&user_email=${author_email}`;

  const url_pieces =
      [ post_url, post_title, post_body, post_url_ending_params ];
  if (author_email) {
